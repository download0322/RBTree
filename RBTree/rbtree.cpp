...
int main(){
	...
	Node   *x;
	int i = 0,n,key;
	printf("请输入需要随机生成的整数个数： ");
	scanf("%d", &n);
	int *input = (int*)malloc(n*sizeof(int));
	srand((unsigned)time(NULL));
	//随机生成n个不同的数，存在数组input中
	while(i<n) {
		int flag = 0;
		int tmp = 0;
		tmp = rand() % 100;
	...
	printf("\n");
	showRBTree(T);
	printf("function 1.请输入需要插入的数字： ");
	scanf("%d", &key);
	rbInsert(&T,key);
	printf("\nresult 1.插入%d后的红黑树为：", key);
	rbPrint(T);
	printf("\n");
	showRBTree(T);
	printf("\nfunction 2.请输入需要删除的数字： ");
	scanf("%d", &key);
	rbDelete(&T, rbSearch(T,key));
	printf("\nresult 2.删除%d后的红黑树为：", key);
	rbPrint(T);
	printf("\n");
	showRBTree(T);
	printf("\n要找第几小元素： ");
...
void rbPrint(RBTree T) 
{
	...
	{
		rbPrint(T->left);
		printf("%d(%s)%d  ", T->key, (T->color == RED) ? "红" : "黑",T->size);
		rbPrint(T->right);
	}
}
...
int Select(RBTree T, int i)
{
	...
	else return Select(T->right, i - r);
